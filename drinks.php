...
<section class="products">
    ...
    <div class="box-container">
        ...
        <?php
        // Fetch the category ID for 'Drinks'
        $category_query = mysqli_query($conn, "SELECT id FROM `categories` WHERE name = 'Drinks'") or die('query failed');
        $category_data = mysqli_fetch_assoc($category_query);
        $category_id = $category_data['id'];

        // Fetch products from the 'Drinks' category
        $select_products = mysqli_query($conn, "SELECT * FROM `products` WHERE category_id = '$category_id' LIMIT 6") or die('query failed');
